  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.data) {
        ...
        setMessage("E-posta gönderildi! Lütfen kontrol edin.");
      }
    } catch (error) {
      if (error.response && error.response.status === 403) {
        ...
        setError("Hata: " + error.response.data.message);
      } else {
    ...
    <main className="main">
      <div className="form-container">
        <form className="form" onSubmit={handleSubmit}>
          ...
          <button className="login-button"type="submit"disabled={isResendDisabled}style={{maxWidth: "200px"}}>
            Kod Gönder
          </button>
        </form>
